    ...
    try {
      ...
      dispatch(
        addTransaction({
          ...
          c_withdraw_amount: amount //contract withdraw amount is amount
        })
      );
    ...
    try {
      ...
      dispatch(
        addTransaction({
          ...
          c_deposit_amount: amount //deposit to contract_balance
        })
      );
...
export const initiateTimedPayment = (account_id, payout_freq, payout_amt) => { //auto/timed payment action, takes in payout_freq and user defined payout_amount
  return async (dispatch) => {
    try {
      const transaction = { //is pushed to database, check routes/transactions
        transaction_date: new Date(),
        payout_freq: payout_freq, //payouts from contract_balance into total_balance at user defined frequency
        deposit_amount: payout_amt //deposit to total_balance depending on user defined amount
      };
      ...
      dispatch(updateAccountBalance(payout_amt, payout_freq, 'locked'));
    } catch (error) {
      error.response && dispatch(getErrors(error.response.data)); //displays errors if any
    }
  };
